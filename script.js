...
function dictionaryUrl(term) {
  ...
  return `https://www.oxfordlearnersdictionaries.com/search/english/?q=${t}`;
}`;
}
...
function applyShire(on){
  document.documentElement.classList.toggle('shire', !!on);
  localStorage.setItem('shire', on ? '1' : '0');
  if (shireBtn) shireBtn.textContent = on ? '🍃 Shire' : '🌿 Shire';
}
applyShire(localStorage.getItem('shire') === '1');
